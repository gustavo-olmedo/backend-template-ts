...
> - **Access token**: short-lived JWT (e.g., 10–20 min). Used on *every* request (`jwt` cookie on web; `Authorization: Bearer` on mobile).
...
**Response**
```json
{ "user": { /* profile */ }, "accessToken": "<ACCESS_JWT>" }
```
...
```
POST ${API}/api/login
{ "email": "...", "password": "...", "appInstanceId": "<UUID>", "platform": "ios|android" }
```
**Response**
```json
{
...
- Exchange with backend:
  ...
  { "idToken": "<GOOGLE_ID_TOKEN>", "appInstanceId": "<UUID>", "platform": "ios|android" }
  ```
...
### Schema (essentials)
- `id` (uuid), `user_id`, `app_instance_id` (uuid), `platform` (`ios|android|web`),  
